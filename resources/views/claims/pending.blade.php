            ...
            @foreach($claims as $claim)
                <tr>
                    ...
                    @php
                        ...
                        $decryptedDebtorName = isset($claim->debtor->name) ? Crypt::decryptString(trim($claim->debtor->name)) : 'No existe';
                    @endphp

                    <td>{{ $decryptedName }}</td>
                    <td>{{ $decryptedDebtorName }}</td>
                    <td>{{ isset($claim->debt) ? number_format($claim->debt->pending_amount, 2,',','.') : '-'  }} &euro;</td>
                    <td>{{ number_format($claim->amountClaimed(), 2,',','.') /* + $claim->debt->partialAmounts()*/ }} &euro;</td>
                    <td>{{ isset($claim->debt) ? number_format($claim->debt->pending_amount - ($claim->amountClaimed()/* + $claim->debt->partialAmounts()*/), 2,',','.') :'-'}} &euro;</td>
                    <td>{{ $claim->getType() }}</td>
                    <td>{{ $claim->getHito() }}</td>
                    <td>
                    <nobr>
